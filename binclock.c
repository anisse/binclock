...
void InitScreen(void)
{
    ...
    screen = SDL_SetVideoMode(6*LED_SIZE, 4*LED_SIZE, 0,SDL_ANYFORMAT|SDL_SWSURFACE/*|SDL_NOFRAME*/|SDL_DOUBLEBUF);
...
int main(int argc, char *argv[])
{
  ...
  while(1)
      ...
      switch(event.type)
      	...
      	case SDL_QUIT :
   	    ...
   	    case SDL_KEYDOWN :
                 ...
                 {            
   	              screen = SDL_SetVideoMode(6*LED_SIZE, 4*LED_SIZE, 0,SDL_ANYFORMAT|SDL_SWSURFACE|SDL_DOUBLEBUF);
   	              //SDL_SetAlpha(screen, SDL_SRCALPHA|SDL_RLEACCEL, 128);
   	              SDL_FillRect(screen, NULL, SDL_MapRGB(screen->format,255,255,255));
   	             }               
        default:;
      ...
      if(!(ptime = convert(hour->tm_hour, hour->tm_min, hour->tm_sec)))
          exit(EXIT_FAILURE);
